...
class WorldLightingController:
    ...
    def __init__(self, canvas):
        self.canvas = canvas;
        self.visibility_map = [[False for i in range(self.canvas.width)] for j in range(self.canvas.height)]
        ...
        for i in range(self.canvas.width):
            for j in range(self.canvas.height):
                ...
                if(world_coord_in_bounds(ij_world[0], ij_world[1])):
                    ...
                    self.canvas.put_tile(i, j, Panelself.canvas.default_canvas_tile)
